    ...
    <div className="flex min-h-screen bg-muted/30">
      ...
      <main className="flex-1 overflow-x-hidden">
        <header className="border-b bg-white px-6 py-4">
          <div className="text-[11px] uppercase tracking-wider text-muted-foreground">Admin / Refund Management</div>
          <h1 className="font-display font-extrabold text-2xl text-primary">Refund Management</h1>
        </header>

        <div className="p-6 space-y-5">
          {/* KPI bar */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <Kpi label="Pending" value={counts.pending} color="bg-orange-100 text-orange-800" />
            <Kpi label="Approved" value={counts.approved} color="bg-green-100 text-green-800" />
            <Kpi label="Rejected" value={counts.rejected} color="bg-red-100 text-red-800" />
            <Kpi label="Total Refunded" value={`₹${counts.totalRefunded.toLocaleString("en-IN")}`} color="bg-accent/10 text-accent" />
          </div>
          ...
          <div className="flex items-center gap-2 max-w-md">
            <div className="relative flex-1">
              ...
              <Input
                ...
                className="pl-8 rounded-none border-2"
              />
            </div>
          </div>

          {/* Table */}
          <div className="bg-white border-2 border-muted overflow-x-auto">
            {q.isLoading ? (
              <div className="flex items-center justify-center py-12">
                <Loader2 className="h-6 w-6 animate-spin text-muted-foreground" />
              </div>
            ) : rows.length === 0 ? (
              <div className="py-12 text-center text-sm text-muted-foreground">No refunds match.</div>
            ) : (
              <table className="w-full text-sm">
                ...
                <tbody>
                  {rows.map((r: any) => (
                    <tr
                      key={r.id}
                      className={`border-t border-muted ${r.is_auto_refund ? "border-l-4 border-l-accent" : ""}`}
                    >
